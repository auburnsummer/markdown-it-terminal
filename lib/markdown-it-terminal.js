...
const renderPlaceholders = (content) => {
  const parts = content.split(/\{\{(.*?)\}\}/);
  return parts.reduce((prev, curr, idx) => {
    // every second part is a token
    if (idx % 2) {
      return prev + styles.reset.open + styles.underline.open + styles.cyan.open +  curr + styles.underline.close + styles.cyan.close + styles.reset.close;
    } else {
      return prev + curr;
    }
  });
...
module.exports = function(md, options) {
  ...
  md.renderer.rules.blockquote_close = function (tokens, idx, options, env) {
    env.in_list = false;
    return newline(0, tokens, idx);
  };


  md.renderer.rules.code_block = function (tokens, idx /*, options, env */) {
    var e = unescape ? unescapeEntities : escapeHtml;
    return newline(1, tokens, idx) + styleOptions.code.open + indentText(renderPlaceholders(e(tokens[idx].content))) + styleOptions.code.close + newline(2, tokens, idx);
  };
  md.renderer.rules.code_inline = function (tokens, idx, options, env) {
    var e = unescape ? unescapeEntities : escapeHtml;
    if (!env.in_list) {
      return "  " + styleOptions.codespan.open + renderPlaceholders(e(tokens[idx].content)) + styleOptions.codespan.close;
    } else {
      return styles.bold.open + e(tokens[idx].content) + styles.bold.close;
    }
  };
  ...
  md.renderer.rules.list_item_open     = function (tokens, idx, options, env) {
    ...
    if ((next.type === 'list_item_close') ||
      ...
      return styleOptions.listitem.open + bullet;
    }
  };
  md.renderer.rules.list_item_close    = function (tokens, idx, options, env) { 
    env.in_list = false;
    return styleOptions.listitem.close + newline(0, tokens, idx); 
  };
